...
// Fetch categories specifically for 'preschool'
$categories_query = "SELECT id, category_name FROM categories WHERE category_name = 'Preschool'";
...
// Fetch video names based on selected category (preschool)
if (isset($_GET['category_id'])) {
    $category_id = intval($_GET['category_id']);
    
    // Ensure the category is 'Preschool' before showing videos
    if ($category_id > 0) {
        // Fetch video titles based on selected category
        $video_query = "SELECT video_title FROM videos WHERE category_id = ? AND category_name = 'Preschool'";
...
if (isset($_POST['video_names'])) {
    ...
    // Redirect to preschoolvideos.php with the selected video names passed in the URL
    header("Location: preschoolvideos.php?videos=" . urlencode($selected_names_string));
    exit;
}
...
<head>
    ...
    <title>Preschool Video Categories</title>
    <style>
        body {
...
<body>
    <header>
        <h1>Kids World: Preschool Video Categories</h1>
    </header>
    <main>
        <!-- Categories -->
