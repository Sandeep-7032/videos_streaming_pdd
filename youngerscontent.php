...
// Fetch categories specifically for 'Youngers'
$categories_query = "SELECT id, category_name FROM categories WHERE category_name = 'Youngers'";
...
// Fetch video names based on selected category (Youngers)
if (isset($_GET['category_id'])) {
    $category_id = intval($_GET['category_id']);
    
    // Ensure the category is 'Youngers' before showing videos
    if ($category_id > 0) {
        // Fetch video titles based on selected category
        $video_query = "SELECT video_title FROM videos WHERE category_id = ? AND category_name = 'Youngers'";
...
// Redirect to youngervideos.php after form submission
if (isset($_POST['video_names'])) {
    ...
    // Redirect to youngervideos.php with the selected video names passed in the URL
    header("Location: youngersvideos.php?videos=" . urlencode($selected_names_string));
    exit;
}
...
<head>
    ...
    <title>Youngers Video Categories</title>
    <style>
        body {
...
<body>
    <header>
        <h1>Kids World: Youngers Video Categories</h1>
    </header>
    <main>
        <!-- Categories -->
